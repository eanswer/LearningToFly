...
void Arrow(const float base, const float length,
    const Eigen::Vector3f direction, Eigen::Matrix3Xf& vertex,
    Eigen::Matrix3Xi& face) {

    Eigen::Matrix3Xf cylinder_vertices;
    Eigen::Matrix3Xi cylinder_faces;
    Cylinder(base, base, length, 16, 1, cylinder_vertices, cylinder_faces);

    Eigen::Matrix3Xf cone_vertices;
    Eigen::Matrix3Xi cone_faces;
    Cone(base + 0.01, 0.07f, 16, 1, cone_vertices, cone_faces);
    Translate(0.0f, 0.0f, length - 0.02f, cone_vertices);

    Eigen::Matrix3Xf arrow_vertices(3, cylinder_vertices.cols() + cone_vertices.cols());
    Eigen::Matrix3Xi arrow_faces(3, cylinder_faces.cols() + cone_faces.cols());
    for (int i = 0;i < cylinder_vertices.cols();++i)
        arrow_vertices.col(i) = cylinder_vertices.col(i);
    for (int i = 0;i < cone_vertices.cols();++i)
        arrow_vertices.col(i + cylinder_vertices.cols()) = cone_vertices.col(i);
    for (int i = 0;i < cylinder_faces.cols();++i)
        arrow_faces.col(i) = cylinder_faces.col(i);
    for (int i = 0;i < cone_faces.cols();++i)
        arrow_faces.col(i + cylinder_faces.cols()) = cone_faces.col(i) 
            + Eigen::Vector3i(cylinder_vertices.cols(),
                cylinder_vertices.cols(), cylinder_vertices.cols());

    Eigen::Quaternionf quat;
    quat.setFromTwoVectors(Eigen::Vector3f::UnitZ(), direction);
    Rotate(quat, arrow_vertices);
