...
Viewer::Viewer()
  ...
  options_.SetPointerOption("camera", NULL);
...
void Viewer::Initialize(const Option& option) {
  ...
  camera_rel_pos_ = camera_pos - rotation_anchor_point_;
  ...
  camera_ = reinterpret_cast<Camera*>(options_.GetPointerOption("camera"));
...
const bool Viewer::UpdateStaticObject(const int idx,
    const Eigen::Matrix3Xf& vertex,
    const Eigen::Matrix3Xi& face, const Option& options) {
    if (idx >= next_object_id_)
        return false;
    delete objects_[idx];
    StaticOpenglShape* new_object = new StaticOpenglShape();
    new_object->Initialize(vertex, face, options);
    objects_[idx] = new_object;
    return true;
}
...
void Viewer::Run() {
  // Load shader programs.
    const std::string shader_folder = std::string(GRAPHICS_CODEBASE_SOURCE_DIR) + "/projects/opengl_viewer/shader/";
  ...
  do {
    ...
    if (camera_) {
        rotation_anchor_point_ = camera_->LookAt();
        Eigen::Vector3f camera_pos = rotation_anchor_point_ + camera_rel_pos_;
        view_matrix_ = LookAt(camera_pos, rotation_anchor_point_, camera_up_);
    }
...
void Viewer::MouseWheelScrollCallback(const float y_offset) {
  ...
  camera_rel_pos_ += -view_matrix_.topLeftCorner(3, 3).transpose() * Eigen::Vector3f::UnitZ() * zoom_speed * y_offset;
